#!/usr/bin/env python3
"""
generate_themes_hf.py

Regenerate FloppyDuck background layers using SD 3.5 Large Turbo via Hugging Face.
Reuses the processing pipeline from all_themes_production.py.
"""
import asyncio, subprocess, os, sys, random, argparse, requests, io
...
# Constants from all_themes_production.py
ENGINE = './pixelart_engine'
BASE_OUT = '../../artifacts/theme_candidates'
...
def generate_image_hf(prompt, model=MODEL_ID, steps=4, guidance=0.0):
...
THEMES = {
    ...
    'london': {
        'layers': [
            ...
            {'name': 'midground3', 'bg_removal': 'topdown', 'bg_tolerance': 45, 'dither': 'fs', 'texture': 2,
             'palette': ['181818', '282828', '383848', '484858', '585868', '181c20'],
    ...
    'roughOcean': {
        'layers': [
            {'name': 'background1', 'bg_removal': 'none', 'dither': 'ordered', 'texture': 2,
             'palette': ['041018', '082028', '14323c', '324850', '4a6068', 'ffffff'],
             'prompt': 'Pixel art stormy ocean sky. Dark slate gray clouds with lightning flashes and heavy rain streaks. Moody and oppressive. 8-bit retro style. Full solid background.'},
            {'name': 'background2', 'bg_removal': 'topdown', 'bg_tolerance': 40, 'dither': 'fs', 'texture': 3,
             'palette': ['0c1820', '183040', '244860', '306080', 'ffffff'],
             'prompt': 'Pixel art distant crashing waves and spray on the horizon. Dark blue turbulent water with white foam caps. In lower 25% only. 8-bit retro style.'},
            {'name': 'background3', 'bg_removal': 'flood', 'dither': 'fs', 'texture': 2,
             'palette': ['a0c0c8', 'c0d8e0', 'd8e8f0', 'ffffff', '80a0a8'],
             'prompt': 'Pixel art sea spray and mist. White foam particles and hazy sea mist drifting. On plain white background. 8-bit retro style.'},
            {'name': 'midground1', 'bg_removal': 'topdown', 'bg_tolerance': 40, 'dither': 'fs', 'texture': 3, 'outline': True,
             'palette': ['0a1a28', '1a3a48', '2a5a68', 'ffffff', 'a08040'],
             'prompt': 'Pixel art battered wooden pirate ship in rough seas. Old ship with torn sails pitching in large waves. On plain white background. In lower 35%. 8-bit retro style.'},
            {'name': 'midground2', 'bg_removal': 'topdown', 'bg_tolerance': 45, 'dither': 'fs', 'texture': 3,
             'palette': ['081420', '102838', '184050', '205868', 'ffffff'],
             'prompt': 'Pixel art massive crashing wave with sea foam. Towering wave curling with white water spray. On plain white background. In lower 30%. 8-bit retro style.'},
            {'name': 'midground3', 'bg_removal': 'topdown', 'bg_tolerance': 45, 'dither': 'fs', 'texture': 2,
             'palette': ['101820', '203040', '304860', 'ffffff', '486070'],
             'prompt': 'Pixel art jagged dark sea rocks with lighthouse. Sharp black rocks being hit by waves, with a distant lighthouse silhouette. On plain white background. In lower 30%. 8-bit retro style.'},
            {'name': 'foreground1', 'bg_removal': 'topdown', 'bg_tolerance': 50, 'dither': 'fs', 'texture': 3,
             'palette': ['0c1418', '182830', '284048', 'ffffff', '40c0ff'],
             'prompt': 'Pixel art close-up churning water and bubbles. Detailed dark water surface with many white foam bubbles and ripples. On plain white background. In lower 25%. 8-bit retro style.'},
            {'name': 'foreground2', 'bg_removal': 'none', 'dither': 'ordered', 'texture': 2, 'h': 20,
             'palette': ['081014', '142028', '20303c', '284048', 'ffffff'],
             'prompt': 'Pixel art turbulent dark water ground. Deep blue-black water surface with white foam ripples. Full coverage strip. 8-bit retro style.'},
            {'name': 'foreground3', 'bg_removal': 'none', 'dither': 'ordered', 'texture': 1, 'h': 25,
             'palette': ['060c10', '0c1820', '122430', 'ffffff', '18b0ff'],
             'prompt': 'Pixel art water with floating debris and foam. Dark water with wooden planks and white foam clusters. Full coverage strip. 8-bit retro style.'},
        ]
    }
}
...
def main():
    ...
    parser.add_argument("--themes", nargs="+", help="Theme names to regenerate (e.g. volcano egypt)")
    parser.add_argument("--layers", nargs="+", help="Specific layers to regenerate (e.g. midground1 midground2)")
    parser.add_argument("--steps", type=int, default=4, help="Inference steps for Turbo model")
    parser.add_argument("--guidance", type=float, default=0.0, help="Guidance scale")
    args = parser.parse_args()

    themes_to_run = args.themes or list(THEMES.keys())
    layers_to_run = args.layers # If None, run all

    print(f"=== FloppyDuck HF Theme Regeneration (SD 3.5 Large Turbo) ===")
    ...
    for tname in themes_to_run:
        ...
        theme_def = THEMES[tname]
        out_dir = os.path.join(BASE_OUT, tname)
        ...
        for ldef in theme_def['layers']:
            ...
            # Skip if not in layers_to_run
            if layers_to_run and name not in layers_to_run:
                exist_path = os.path.join(out_dir, f"{name}.png")
                if os.path.exists(exist_path):
                    layer_paths.append((name, exist_path))
                continue
                
            print(f"  🎨 Generating {name}...")
            
            try:
                img = generate_image_hf(ldef['prompt'], steps=args.steps, guidance=args.guidance)
                
                # Background removal
                temp_raw = os.path.join(out_dir, "temp", f"{name}_raw.png")
                os.makedirs(os.path.dirname(temp_raw), exist_ok=True)
                img.save(temp_raw)
                
                bg_mode = ldef.get('bg_removal', 'none')
                if bg_mode == 'topdown':
                    img_proc = remove_bg_topdown(temp_raw, tolerance=ldef.get('bg_tolerance', 50))
                elif bg_mode == 'flood':
                    img_proc = remove_bg_flood(temp_raw)
                else:
                    img_proc = Image.open(temp_raw).convert('RGBA')
                ...
                path = process_layer(
                    img_proc, name, ldef['palette'], lw, lh, out_dir,
        ...
        if layer_paths:
            comp_path = make_composite(layer_paths, out_dir, tname)
            print(f"  ✅ {tname} composite: {comp_path}")
