...
- `docs/launch/TESTFLIGHT_RUNBOOK.md` - launch-readiness runbook
- `docs/APPSTORE_METADATA.md` - App Store copy and IAP inventory draft
- `docs/art/ARTWORK.md` - current intended art direction, though it needs runtime-path
  reconciliation
- `prompts/README.md` and `prompts/*.md` - current hero prompt format
- `docs/product/tracking.md` - analytics plan and PostHog implementation notes
- `docs/launch/AUTH_SETUP.md` and `convex/README.md` - auth/backend setup notes
...
- `docs/product/growth.md`
- `docs/product/monetization.md`
- `docs/product/support-playbook.md`
- `docs/product/research.md`

Needs cleanup or archival:

- `docs/archive/PROCESS.md`
- `docs/archive/notebook.md`
...
`docs/art/ARTWORK.md` says the direction is one generated hero image plus deterministic
...
Recommended action:
- make `docs/art/ARTWORK.md` explicitly name `ThemeRecipeCatalog` and `ParallaxManager`
...
`docs/launch/TESTFLIGHT_RUNBOOK.md` also says all 7 premium products are registered. The local
...
- update `docs/product/monetization.md` after the decision
...
CC0 music packs, `docs/art/THIRD_PARTY_ASSETS.md` tracks an imported CC0 birds sprite, and
...
- keep third-party asset provenance in `docs/art/THIRD_PARTY_ASSETS.md`
- avoid claims that every visual and sound is code-generated unless the repo
  actually returns to that model

### 4. `docs/archive/PROCESS.md` Is Historical And Stale

`docs/archive/PROCESS.md` describes early development, old constraints, and future plans such
...
### 5. `docs/archive/notebook.md` Mixes Architecture, Reflections, And Old Roadmap

`docs/archive/notebook.md` is valuable for context but too large and mixed-purpose. It includes
architecture notes, gotchas, old "this commit" summaries, and historical
reflections. Some sections are still useful; others duplicate `docs/product/ROADMAP.md`,
`docs/launch/TESTFLIGHT_RUNBOOK.md`, and current code comments.
...
- keep `docs/archive/notebook.md` as a short pointer file or remove it from the main docs map
...
and linked from `docs/launch/TESTFLIGHT_RUNBOOK.md` only if it is still planned.
...
- update `docs/launch/TESTFLIGHT_RUNBOOK.md` to state the actual screenshot CI state

### 7. README Status Date Is Stale

`README.md` says "Current Status (April 2, 2026)" while `docs/launch/TESTFLIGHT_RUNBOOK.md` says
...
- replace date-specific status with "Current Status" plus links to canonical
  status files
- keep launch state in `docs/launch/TESTFLIGHT_RUNBOOK.md`
...
1. Fix the IAP inventory mismatch in `docs/APPSTORE_METADATA.md`,
   `docs/launch/TESTFLIGHT_RUNBOOK.md`, `docs/product/monetization.md`, StoreKit config, and visible shop state.
2. Rewrite the theme docs around the current recipe-driven SpriteKit render path.
3. Mark `docs/archive/PROCESS.md`, old parts of `docs/archive/notebook.md`, and old pipeline docs as
