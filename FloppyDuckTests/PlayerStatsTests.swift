...
final class PlayerStatsTests: XCTestCase {
    ...
    func testWinRateCalculation() {
        ...
        stats.rankedWins = 2
        stats.rankedLosses = 2

        // 2 ranked wins / 4 ranked games = 0.5
    ...
    func testRecordGameWithWinAwardsBread() {
        // Completed runs award one loaf, regardless of win/loss outcome.
        var stats = PlayerStats()
        stats.recordGame(score: 10, won: true)
        XCTAssertEqual(stats.bread, 1)

        var stats2 = PlayerStats()
        stats2.recordGame(score: 1, won: true)
        XCTAssertEqual(stats2.bread, 1)
    }
    ...
    func testRecordGameWithLossAwardsBread() {
        ...
        XCTAssertEqual(stats2.bread, 0)
    }

    func testRecordGameClassicAwardsBread() {
        // Classic completed runs award one loaf.
        var stats = PlayerStats()
    ...
    func testRecordGameAddsCollectedBreadToSpendableBalance() {
        ...
        XCTAssertEqual(stats.bread, 1)
        XCTAssertEqual(stats.totalBreadCollected, 1)
    }

    func testBreadEconomyUsesSameScoreRewardFormula() {
        XCTAssertEqual(BreadEconomy.scoreReward(score: 10, won: true), 1)
        XCTAssertEqual(BreadEconomy.scoreReward(score: 10, won: false), 1)
        XCTAssertEqual(BreadEconomy.scoreReward(score: 0, won: nil), 0)
    }
